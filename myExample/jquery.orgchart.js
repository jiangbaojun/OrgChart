...
(function(factory) {
  ...
  $.fn.orgchart = function(options) {
    //处理pid数据
    if(options.data && options.isPidData){
      options.data=transformPidData(options.data,{id:options.rootId});
    }
    var defaultOptions = {
      'nodeTitle': 'name',
      'nodeId': 'id',
   ...
   * 转换pid类型数据
   * @param data pid数据
   * @param object  递归数据，初始值格式{id:idValue}
   * @returns {*} 返回树形数据
   */
  function transformPidData(data, object) {
    if(!data || !object || !object.id){
      return {};
    }
    //完善根节点
    $.each(data, function (i, element) {
      if(element.id == object.id && !element.parentId){
    ...
    if(childNodes!=null && childNodes!=undefined && childNodes.length>0){
      object.children = [];
      $.each(childNodes, function (i, element) {
        var node = {};
        $.each(element,function(name, value){
          node[name]=value;
        });
